...
export default function PcMyInvitationsPage() {
  ...
  return (
    <RequireAuth nextPath="/pc/my-invitations">
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '2rem 1.5rem 3rem' }}>
        ...
        {(status === 'empty' || (status === 'ready' && items.length === 0)) && (
          <div style={{ padding: 40, textAlign: 'center', background: '#fff', borderRadius: 20 }}>
            ...
            <Link href="/pc/templates" style={{ color: '#4F46E5', fontWeight: 600 }}>
              첫 초대장 만들기
            </Link>
          </div>
        )}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <Link href={`/pc/editor/${item.id}`} style={{ color: '#4F46E5', fontWeight: 600, fontSize: 13 }}>
          수정
        </Link>
        {item.shareSlug ? (
